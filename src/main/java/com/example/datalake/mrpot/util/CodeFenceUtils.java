...
public final class CodeFenceUtils {
  ...
  public static class Segment {
    ...
    public Segment(String t, boolean c) {
      text = t;
      isCode = c;
    }
  }
  ...
  public static List<Segment> split(String input) {
    ...
    Matcher m = CODE_FENCE.matcher(input);
    int last = 0;
    while (m.find()) {
      if (m.start() > last)
        out.add(new Segment(input.substring(last, m.start()), false));
      out.add(new Segment(input.substring(m.start(), m.end()), true));
      last = m.end();
    }
    if (last < input.length())
      out.add(new Segment(input.substring(last), false));
    return out;
  }
