 ...
 * 3) Rewrite system prompt but preserve the original (saved in ctx metadata if available; otherwise
 * no-op). 4) Enrich prompts with keyword list (top-N keywords) for better retrieval / grounding.
 *
...
public class PromptTemplateProcessor implements TextProcessor {
  ...
  public Mono<ProcessingContext> process(ProcessingContext ctx) {
    return Mono.fromSupplier(() -> {
      ...
      if (!keywords.isEmpty()) {
        ...
        if (isBlank(newSystem))
          newSystem = "";
        if (isBlank(newUser))
          newUser = safeReadUserText(ctx);
  ...
  private String pickLanguage(ProcessingContext ctx) {
    ...
    String lang = tryGetString(
        ctx, "getIndexLanguage", "getLanguageCode", "getLanguage", "language", "indexLanguage");
    if (isBlank(lang))
      lang = DEFAULT_LANG;
  ...
  private Map<String, Object> buildVarsFromContext(
      ProcessingContext ctx, String lang, String intent) {
  ...
  private String safeReadUserText(ProcessingContext ctx) {
    return firstNonBlank(tryGetString(ctx, "getUserPrompt", "userPrompt"),
  ...
  private void trySetString(
      ProcessingContext ctx, String setterName, String fieldName, String value) {
    // Try setter first
    try {
    ...
    log.warn("Unable to write '{}' on ProcessingContext (setter '{}' or field '{}').", fieldName,
  ...
  private record TemplateIndex(Map<String, Map<String, PromptTemplate>> idx) {
    static TemplateIndex empty() {
      return new TemplateIndex(new HashMap<>());
    }
    int languages() {
      return idx.size();
    }
  ...
  private static final class Json {
    ...
    private Json(Object node) {
      this.node = node;
    }
    static Json parse(String s) {
      return new Json(new Parser(s).parseValue());
    }
    boolean isObject() {
      return node instanceof Map;
    }
    Set<String> keys() {
      return isObject() ? ((Map<String, Object>) node).keySet() : Collections.emptySet();
    }
    Json get(String k) {
      if (!isObject())
