...
public class UnifiedCleanCorrectProcessor implements TextProcessor {
  @Override
  public String name() {
    return "unified-clean-correct";
  }

  // ====== Precompiled patterns (fast path) ======
  private static final Pattern SENT_SPLIT = Pattern.compile("[\\n；;。.!?]+\\s*");
  private static final Pattern CN_EXCESS_SPACES =
      Pattern.compile("(?<=\\p{IsHan})\\s+(?=\\p{IsHan})");
  private static final Pattern CN_WORD_SPACE =
      Pattern.compile("(?<=\\p{IsHan})\\s+(?=[A-Za-z0-9])|(?<=[A-Za-z0-9])\\s+(?=\\p{IsHan})");
  private static final Pattern CN_REPEAT_CHAR =
      Pattern.compile("([\\p{IsHan}！？。；，、])\\1{1,}");
  ...
  public Mono<ProcessingContext> process(ProcessingContext ctx) {
    ...
    if (condensed.length() > limit) {
      condensed =
          condensed.substring(0, limit - 50) + "\n[Content condensed to enforce length limit]";
    }

    // 6) Write back
    double changeRatio =
        raw.isEmpty() ? 0.0 : Math.abs(condensed.length() - raw.length()) / (double) raw.length();
    ...
    return Mono.just(
        ctx.addStep(name(), "ratio=" + String.format(Locale.ROOT, "%.3f", changeRatio)));
  }
  ...
  private static String normalizeText(String s) {
    ...
    s = s.replaceAll("[“”]", "\"").replaceAll("[‘’]", "'").replace("—", "-").replace("–", "-");
