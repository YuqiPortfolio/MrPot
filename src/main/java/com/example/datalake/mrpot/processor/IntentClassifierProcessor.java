...
public class IntentClassifierProcessor implements TextProcessor {
  ...
  private static final Set<String> GREETING_PHRASES = Set.of("hi", "hi there", "hello",
      "hello there", "hey", "hey there", "greetings", "howdy", "hola", "good morning",
      "good afternoon", "good evening", "good day", "morning", "evening", "你好", "您好");
  ...
  public Mono<ProcessingContext> process(ProcessingContext ctx) {
    ...
    Set<String> tags =
        new LinkedHashSet<>(Optional.ofNullable(ctx.getTags()).orElseGet(LinkedHashSet::new));
    ...
    Optional<PromptTemplate> tpl =
        templateFinder.findTopByIntentAndLanguage(predicted.name().toLowerCase(Locale.ROOT),
            Optional.ofNullable(ctx.getIndexLanguage()).orElse("en"));
    tpl.ifPresent(ctx::setTemplate);

    // 6) step log
    String note = "intent=" + predicted + (matchedRule != null ? (", rule=" + matchedRule) : "")
  ...
  private boolean isGreetingText(String text) {
    ...
    for (int i = 0; i < lower.length(); i++) {
      ...
      if ((c >= 'a' && c <= 'z') || Character.isWhitespace(c) || c == '\u4f60' || c == '\u597d'
          || c == '\u60a8') {
        cleaned.append(c);
      }
    }
  ...
  private List<Rule> loadRules(ResourceLoader rl, String classpathName) {
    try (InputStream in = openForRead(rl, classpathName, "intent.rules", "INTENT_RULES_PATH",
  ...
  private Map<String, Set<String>> loadLexicon(ResourceLoader rl, String classpathName) {
    ...
    try (InputStream in = openForRead(rl, classpathName, "keywords.map.path", "KEYWORDS_MAP_PATH",
             "src/main/resources/" + classpathName)) {
      if (in == null) {
  ...
  private InputStream openForRead(ResourceLoader rl, String classpathName, String sysPropKey,
      String envKey, String lastResortPath) {
    try {
    ...
    try {
      ...
      InputStream in =
          IntentClassifierProcessor.class.getClassLoader().getResourceAsStream(classpathName);
      if (in != null)
        return in;
    } catch (Exception ignore) {
    }
    ...
    try {
      ...
      String external =
          Optional.ofNullable(System.getProperty(sysPropKey)).orElse(System.getenv(envKey));
  ...
  private List<Rule> toRules(RuleBundle bundle) {
    ...
    for (RuleJson rj : bundle.rules) {
      ...
      rs.add(new Rule(intent, (rj.minScore == null ? 1 : rj.minScore), toLowerList(rj.any),
          toLowerList(rj.all), toLowerList(rj.none), toLowerList(rj.tagsBoost),
