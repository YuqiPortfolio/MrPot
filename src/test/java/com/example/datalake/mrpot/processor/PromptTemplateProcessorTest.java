...
public class PromptTemplateProcessorTest {
    ...
    private static void setProp(Object target, String prop, Object value) {
        ...
        for (Method m : cls.getMethods()) {
            if (!m.getName().equals("set" + up(prop))) continue;
    ...
    private static Object coerce(Object v, Class<?> targetType) {
        ...
        if (List.class.isAssignableFrom(targetType) && v instanceof List) return v;
        return v; // best effort
    }

    private static String up(String s) { return s == null || s.isEmpty() ? s : Character.toUpperCase(s.charAt(0)) + s.substring(1); }
    ...
    void exactTemplate_en_qa_withKeywordEnrichment_andSystemPreserve() {
        ...
        setProp(ctx, "keywords", Arrays.asList(
                "Elasticsearch", "shards", "index", "primary shards", "replicas",
                "cluster", "routing", "throughput", "ES", "ILM", "hot-warm-cold"
        ));
        ...
        assertTrue(sys.contains("You answer questions precisely"), "expected en/qa system template content");
    ...
    void fallback_zh_coding_to_zh_general_and_enrichment() {
        ...
        setProp(ctx, "intent", "coding"); // zh/coding missing in test templates → fallback to zh/general
    ...
    void placeholderRender_and_topKCap_and_languageNormalization() {
        ...
        setProp(ctx, "keywords", Arrays.asList(
                "a", "ab", "microservices", "distributed systems", "observability",
                "cache invalidation", "CI/CD", "immutability", "DDD",
                "zero-downtime deployments", "TLS", "idempotency"
        ));
